...
- **`cogs/models.py`** — `model` bridge command (`ModelView`: one `discord.ui.Select` each for chat and image model, options from the curated lists in `chatbotfunc/model_settings.py` plus a "Default" that clears the override; a pick edits the embed in place and posts a public switch notice) and `usage` bridge command (`build_usage_embed`: today/7d/30d spend + 30-day by-feature and by-model breakdowns from `chatbotfunc/usage.summarize`).
...
- **`chatbotfunc/model_settings.py`** — `CHAT_CHOICES` / `IMAGE_CHOICES` (curated; each verified live against the bot's call shapes — gpt-6-astra is excluded because it rejects `reasoning_effort="none"`), `get_chat_model()` / `get_image_model()` (override → `.env` → built-in default; stale overrides for de-listed models are ignored), `set_model(kind, model|None, set_by)`.
...
|---|---|---|
| `!generate <prompt>` | `/generate` | Generate an image (current image model — `/model`) |
...
| `!model` | `/model` | See/switch chat and image models (dropdowns, bot-wide) |
