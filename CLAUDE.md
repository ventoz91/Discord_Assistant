...
- **`cogs/models.py`** — `model` bridge command (`ModelView`: one `discord.ui.Select` each for chat model, image model and image size, options from the curated lists in `chatbotfunc/model_settings.py` plus a "Default" that clears the override; a pick edits the embed in place and posts a public switch notice) and `usage` bridge command (`build_usage_embed`: today/7d/30d spend + 30-day by-feature and by-model breakdowns from `chatbotfunc/usage.summarize`).
...
| `!model` | `/model` | See/switch chat model, image model and image size (dropdowns, bot-wide) |
