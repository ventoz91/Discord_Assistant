...
- `MODEL_CHAT` — OpenAI model for chat completions (e.g. `gpt-6-sol`). `/model` in Discord can override it at runtime (`data/model_settings.json`, read via `chatbotfunc/model_settings.get_chat_model()`); background jobs keep their own `*_MODEL` settings and don't follow a switch.
- `IMAGE_MODEL` — image model for generate/transform (default: `gpt-image-1`), also overridable via `/model`.
- `USAGE_RETENTION_DAYS` — days of per-feature token/cost history kept in `data/usage.json` for `/usage` (default: 90).
...
- `IMAGE_SIZE` — Default image dimensions for generation: `1024x1024` / `1536x1024` / `1024x1536` / `auto` (default: `1024x1024`). Overridable via `/model`; a single request can still pick a shape (`/generate aspect:`, `!generate --landscape`, or the AI tool's `aspect` arg). Transforms ignore it and keep the source image's orientation (`model_settings.aspect_for`).
- `IMAGE_QUALITY` — `low` / `medium` / `high` (default: `medium`). Overridable via `/model`, which deliberately offers only low/medium (high is ~4× medium's cost); `high` is still reachable via `.env`.
...
- `DEBATE_SCAN_MAX_TOKENS` — Max tokens for the extraction response (default: 600).
...
- `MINECRAFT_MODDED_SSH_HOST` / `MINECRAFT_MODDED_SSH_USER` / `MINECRAFT_MODDED_SERVICE` (default `minecraft-modded`) — modded runs as a systemd *user* unit on the desktop PC (10.13.37.150), started over SSH (`systemctl --user start`); the key there is scoped by `deploy/bot-ssh-gate-systemd.sh`.
- `MINECRAFT_MODDED_RCON_HOST` / `MINECRAFT_MODDED_RCON_PORT` / `MINECRAFT_MODDED_RCON_PASSWORD` — status and stop (stop falls back to `systemctl --user stop` if RCON is unreachable)
...
- `DRAGONWILDS_SSH_HOST` / `DRAGONWILDS_SSH_USER` / `DRAGONWILDS_COMPOSE_DIR` — SSH+Docker control for RuneScape: Dragonwilds on GameDocker (default compose dir `/home/data`, same shape as Palworld); `DRAGONWILDS_CONNECT_URL` — free-text connect address. No pull_and_redeploy support — the compose service uses `build:` (custom image), not a pulled one, and the container's entrypoint runs DepotDownloader on every start, so a plain restart already updates the game.
- `DRAGONWILDS_CHANNEL_ID` — channel where the persistent Dragonwilds status panel (`DragonwildsPanel`, `persistent=True`) is posted on `on_ready` and kept auto-refreshed; also where prefix/slash start/stop commands (not the panel's own buttons, which already update visibly in that channel) announce. Leave unset to disable both.
...
- **`cogs/images.py`** — `generate`, `transform`, `image` commands. `generate` and `transform` have separate prefix and slash implementations (see Slash vs Prefix).
...
- **`cogs/servers.py`** — `minecraft`/`satisfactory` bridge commands; EmuCoach `/emucoach start|stop|status` slash group; Dragonwilds prefix (`!start_dragonwilds`/`!stop_dragonwilds`/`!dragonwilds_status`) + `/dragonwilds start|stop|status` slash group, same shape as EmuCoach (no bare `/dragonwilds` command — would collide with the subcommand group); `panel` bridge command (posts `WEBPANEL_URL`, ephemeral). Also owns an `on_ready` listener (separate from `ChatCog`'s — discord.py dispatches to every cog's listener) that posts/refreshes a persistent `DragonwildsPanel` in `DRAGONWILDS_CHANNEL_ID` if set: registers the view via `bot.add_view()` so its buttons survive restarts, and finds any existing panel message by scanning the channel's recent history for the bot's own embed (matched by title) rather than persisting a message ID to disk.
- **`cogs/models.py`** — `model` bridge command (`ModelView`: one `discord.ui.Select` each for chat model, image model, image size and image quality, options from the curated lists in `chatbotfunc/model_settings.py` plus a "Default" that clears the override; a pick edits the embed in place and posts a public switch notice) and `usage` bridge command (`build_usage_embed`: today/7d/30d spend + 30-day by-feature and by-model breakdowns from `chatbotfunc/usage.summarize`).
...
- **`generate`**: prefix takes an optional leading `--landscape`/`--portrait`/`--square` flag; slash has an explicit `aspect` choice option
...
- **`chatbotfunc/profiles.py`** — `get_user_context(user_id, display_name) -> str | None`: reads `data/user_profiles.json`, returns formatted fact string for system prompt injection. `extract_and_update(user_id, display_name, user_msg, bot_msg, model)`: async background task; sends extraction prompt to LLM with a strict JSON-schema `response_format` (`{"facts": [...]}`), merges new facts into profile file; truncated (`finish_reason=length`) responses are logged and dropped.
...
- **`chatbotfunc/model_settings.py`** — `CHAT_CHOICES` / `IMAGE_CHOICES` (curated; each verified live against the bot's call shapes — gpt-6-astra is excluded because it rejects `reasoning_effort="none"`), `SIZE_CHOICES` (image size, a third override kind `"size"`; every image model verified at every size, generate + edit, 2026-09-25), `ASPECTS` (square/landscape/portrait → size, for per-request shapes), `aspect_for(w, h)`, `QUALITY_CHOICES` (low/medium only, kind `"quality"`). `get_chat_model()` / `get_image_model()` / `get_image_size()` / `get_image_quality()` (override → `.env` → built-in default; stale overrides for de-listed values are ignored), `set_model(kind, value|None, set_by)`.
- **`chatbotfunc/usage.py`** — `record_chat(model, usage, feature)` is called by `async_chat_completion` for every chat call (callers pass `usage_tag=`: chat, vision, profiles, debates, summaries, morning_paper, reminders, summarize, missed, simulate, announcements; untagged → other); `record_image` by `generate_image`/`transform_image`. Per-day/feature/model totals in `data/usage.json`; costs from the `CHAT_PRICES`/`IMAGE_PRICES` list-price tables (update when prices change). `summarize(days)` feeds `/usage`. Tests isolate its store via an autouse fixture in `tests/conftest.py`.
- **`chatbotfunc/debates.py`** — `debate_scanner_loop(bot, model)`: background coroutine; `scan_channel(bot, channel_id, model)`: pulls channel history since the last scan directly from Discord, sends it plus the existing tracked list to an LLM extraction prompt, applies returned `new`/`update`/`resolve` actions (strict JSON-schema `response_format`, `{"actions": [...]}` with unused fields null) to `data/debates.json`, caps entries per channel. A truncated response leaves `last_scan_ts` unadvanced so the next scan retries the window. `get_debate_context(channel_id) -> str | None`: formats unresolved entries not surfaced within the cooldown window for system-prompt injection. `mark_surfaced(channel_id, response_text)`: fuzzy-matches topic words against a sent response and stamps `last_surfaced_ts` so the same callback isn't repeated immediately.
...
- **`gamefunc/minecraft.py`** — Thread-safe async RCON using `socket.settimeout()` (avoids `signal.alarm()` crash outside main thread). `MinecraftServer` handles vanilla/creative (SSH+Docker via `DockerComposeGameServer`, one instance per type built in `__init__` from `MINECRAFT_{TYPE}_*` env vars) and modded (`RemoteUserService` from `gamefunc/user_service.py` — a systemd user unit on the desktop PC; no proxy, no redeploy). `pull_and_redeploy(server_type)` — vanilla/creative only. `start()` also starts the proxy (`MINECRAFT_PROXY_SERVICE`); `stop()` (RCON `stop`) leaves it running. Modded `stop()` = RCON `stop` (saves the world), wait for RCON to go down, then `systemctl --user stop` — a mod thread keeps the JVM alive after the save otherwise. Verified end to end 2026-09-25: ready in ~30s, stop ~10s. The PC must not route LAN replies through WireGuard (see README Scoped SSH access).
...
- **`gamefunc/user_service.py`** — `RemoteUserService(host_env, user_env, unit_env, default_unit)`: `systemctl --user start|stop|is-active <unit>` over SSH, env read per call. Used for modded Minecraft on the desktop PC.
...
- **`gamefunc/dragonwilds.py`** — `DragonwildsServer`: same shape as `PalworldServer` (service and container are both `dragonwilds`, no status API). No `pull_and_redeploy` — see env var docs above for why.
...
- **`deploy/bot-ssh-gate-systemd.sh`** — companion gate for the desktop PC (installed at `~/.local/bin/bot-ssh-gate-systemd`, unit names as args in the `authorized_keys` line): only `systemctl --user start|stop|is-active <listed unit>`. `tests/test_ssh_gate_systemd.py` generates its allowed commands from the real `RemoteUserService`. Unit file: `deploy/minecraft-modded.service` (no `[Install]` — on-demand only; needs linger).
- **`scripts/prod-env.sh`** — `show|set|unset KEY[=VALUE]` against the deployed `.env` on local1 over SSH: timestamped backup, in-place replace (duplicates dropped), `docker compose up -d`, then prints the container's resulting values with secret-looking keys masked. Values travel base64-encoded to a fixed remote Python program, never through a remote shell. Use this (not ad-hoc ssh) for production config changes.
- **`funfunc/`** — `image_search.py` (Google CSE), `web_search.py` (Tavily, used by `google_search` AI tool), `sandwich.py`.
- **`webpanel/`** — browser admin panel, runs in-process with the bot (see README "Web Panel" for setup/usage). `app.py` (FastAPI factory, session middleware), `auth.py` (single-admin login, `require_login` dependency, in-memory per-IP lockout after 5 failed logins in 15 min), `routes_servers.py` (dashboard for fixed servers, wraps the same `gamefunc/` classes cogs use; Minecraft rows append the proxy's online state; Modded and Dragonwilds rows have no redeploy), `routes_deploy.py` (template catalog, deploy/manage/delete instances), `templates_catalog.py` (curated templates + `render_compose_yaml`), `store.py` (`data/deployed_servers.json`), `templates/`+`static/` (Jinja2 + htmx, no JS build step).
...
3. Image path (`_analyze_and_reply`): attachments are detected by MIME `content_type` (falling back to `IMAGE_EXTENSIONS` / `VIDEO_EXTENSIONS` filename match); video attachments are analyzed via `VIDEO_FRAMES` ffmpeg-sampled frames sent together to the vision call (`analyze_image` accepts a single base64 string or a list); lone-URL messages (GIF picker → Tenor/Giphy) are vision-analyzed via their embed's preview image (`_embed_image_source`, which waits 2s and re-fetches once if Discord hasn't unfurled the embed yet). Sends response, stores user + analysis to RAG with message IDs, returns. If the media can't be fetched/decoded, falls through to the text path.
...
|---|---|---|
| `!generate [--landscape\|--portrait\|--square] <prompt>` | `/generate [aspect]` | Generate an image (current image model and default size — `/model`) |
...
| `!start_dragonwilds` / `!stop_dragonwilds` / `!dragonwilds_status` | `/dragonwilds start\|stop\|status` | Manage RuneScape: Dragonwilds on GameDocker (SSH+Docker); persistent status panel auto-posted in `DRAGONWILDS_CHANNEL_ID` |
...
| `!model` | `/model` | See/switch chat model, image model, image size and quality (dropdowns, bot-wide) |
| `!usage` | `/usage` | Estimated OpenAI spend: today / 7d / 30d, by feature and model |
