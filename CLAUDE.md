...
- **`gamefunc/minecraft.py`** — Thread-safe async RCON using `socket.settimeout()` (avoids `signal.alarm()` crash outside main thread). `MinecraftServer` handles vanilla/creative (SSH+Docker via `DockerComposeGameServer`, one instance per type built in `__init__` from `MINECRAFT_{TYPE}_*` env vars) and modded (`RemoteUserService` from `gamefunc/user_service.py` — a systemd user unit on the desktop PC; no proxy, no redeploy). `pull_and_redeploy(server_type)` — vanilla/creative only. `start()` also starts the proxy (`MINECRAFT_PROXY_SERVICE`); `stop()` (RCON `stop`) leaves it running.
